...
//		usefulMethods();
	...
	//	StringBuffer : 버퍼 기반으로 작동 -> 복잡한 문자열을 만들어내고자 할 때 좀 더 메모리 효율이 좋음
	private static void stringBufferEx() {
		//	버퍼 생성(버퍼 길이 10, 실제 길이 4로 자동생성됨)
	...
	private static void usefulMethods() {
		...
		//	분할 : split
		String[] chunks = str.split(" ");	//	공백을 기준으로 문자열을 ㅜㅂㄴ할
		for (String chunk: chunks) {
			System.out.println("WORD: " +chunk);
		}
		
		//	문자열 비교: Unicode 비교
		//	같으면 0, 앞의 것이 작으면(먼저면) 음수, 앞의 것이 크면(뒤면) 양수
